...
struct CaptionView: View {
    ...
    var body: some View {
        GeometryReader { _ in
            ZStack(alignment: .center) {
                VStack(spacing: 0) {
                    CustomNavigationBar(
                        leadingType: .none,
                        centerType: .timeTitle(
                            title: DateUtils.string(from: .now, format: .monthDay),
                            timeImage: DateUtils.isATime(date: .now) ? "sun.max.fill" : "moon.fill"
                        ),
                        trailingType: .close(action: { isShowCancelAlert = true }),
        ...
        .customAlert(
            ...
            title: "나가시겠어요?",
            message: "지금까지 찍은 사진은 사라져요",
            confirmTitle: "나가기",
            cancelTitle: "취소",
            onConfirm: { onReturnToHome() },
            onCancel: {}
        )
    }
    ...
    private var captionInputSection: some View {
        VStack(alignment: .center, spacing: 0) {
            HStack {
                ...
                ZStack(alignment: .center) {
                    TextField(
                        DateUtils.isATime(date: .now) ? "오늘 나의 오전을 설명해 주세요..." : "오늘 나의 오후를 설명해 주세요...",
